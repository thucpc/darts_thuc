...
class DataEmbedding_inverted(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.value_embedding = nn.LazyLinear(d_model)
...
class _TransformerModule(PLPastCovariatesModule):
    def __init__(
        ...
        **kwargs
        ...
        self.src_emb=DataEmbedding_inverted(self.input_chunk_length,d_model=self.input_chunk_length)

        self.tgt_emb=DataEmbedding_inverted(self.output_chunk_length,d_model=self.output_chunk_length)
    ...
    def forward(self, x_in: Tuple):
        ...
        src, tgt = data.permute(1, 0, 2)

        # "math.sqrt(self.input_size)" is a normalization factor
        # see section 3.2.1 in 'Attention is All you Need' by Vaswani et al. (2017)
        src = self.encoder(self.src_emb(src)) * math.sqrt(self.input_size)
        src = self.positional_encoding(src)

        tgt = self.encoder(self.tgt(tgt)) * math.sqrt(self.input_size)
...
from darts.layers.Transformer_EncDec import Encoder, EncoderLayer
from darts.layers.SelfAttention_Family import FullAttention, AttentionLayer
# from darts.layers.Embed import DataEmbedding_inverted
import numpy as np

class _iTransformerModel(PLPastCovariatesModule):
    """
    ...
    def __init__(        
        ...
        self.enc_embedding = DataEmbedding_inverted(
            self.input_dim, self.d_model, self.embed, self.frep, self.dropout)
        # Encoder
    ...
    def forecast(self, x_enc):
        ...
        dec_out = dec_out[:,:,0,None]
        dec_out = dec_out[:,:,:,None]
        return dec_out
...
class iTransformerModel(PastCovariatesTorchModel):
    ...
    def _create_model(self, train_sample: Tuple[torch.Tensor]) -> torch.nn.Module:
        ...
        output_dim = train_sample[-1].shape[0]
        nr_params = 1 if self.likelihood is None else self.likelihood.num_parameters

        return _iTransformerModel(
            input_dim=input_dim,
