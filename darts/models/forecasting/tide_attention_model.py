...
class _TideModule(PLMixedCovariatesModule):
    def __init__(
        ...
        **kwargs
        ...
        self.encoders_out=nn.LazyLinear(hidden_size)
    ...
    def forward(
        ...
        encoded = [
            ...
            x_dynamic_past_covariates,
            # x_dynamic_future_covariates,
            x_dynamic_future_covariates[:,:x_dynamic_past_covariates.shape[1],:],
            # x_static_covariates,
        ]
        ...
        encoded = self.encoders(self.src_emb(encoded))
        encoded = encoded[0]
        encoded = [
            encoded,
            x_static_covariates,
        ...
        encoded = [t.flatten(start_dim=1) for t in encoded if t is not None]
        encoded = torch.cat(encoded, dim=1)
        encoded = self.encoders_out(encoded)
